...
const QuizSummary = () => {
    const { questions, userAnswers } = useAppSelector(getQuiz);
    const currentAns = questions.reduce((count, qn, i) => {
        return qn.correctAnswer === userAnswers[i] ? count + 1 : count;
    }, 0)
    const correctPercentage = parseFloat(((Number(currentAns) / Number(questions.length)) * 100).toFixed(2));
        ...
        <div className="flex justify-center">
            <Card className="w-[400px] rounded-md">
                ...
                <CardContent className="space-y-3">
                    <h4 className="font-bold">You got {currentAns} out of {questions.length}!</h4>
                    ...
                    <h3 className="text-red-500">Incorrect Answer: {questions.length - currentAns}</h3>
